...
def prompt_batch_confirm(ctx: SheetContext) -> bool:
    ...
    num_data_rows = len(ctx.sheet_data) - (ctx.id_cell.row + 1) - (1 if ctx.filename_row_idx is not None else 0)
...
def prompt_range_selection(ctx: SheetContext) -> Optional[Tuple[int, int]]:
    ...
    while True:
        ...
        if spreadsheet._validate_row_range(start_line, end_line, max_row) is None:
            continue
        utils.info_print(f'Lines selected: {ctx.sheet_data[start_line-1][ctx.observation_cell.col-1]} to {ctx.sheet_data[end_line-1][ctx.observation_cell.col-1]}')
    ...
    while True:
        try:
            ...
            for participant_id, row_number in parsed_specs:
                col_idx = spreadsheet.find_participant_column(ctx.header_row, ctx.id_cell, participant_id)
                if col_idx is None:
...
def prompt_participant_selection(ctx: SheetContext) -> Optional[List[str]]:
    ...
    """
    available_list = spreadsheet.get_participant_list(ctx.header_row, ctx.id_cell, ctx.num_participants)
    ...
    while True:
        ...
        for token in tokens:
            if token.isdigit():
                ...
                col_idx = spreadsheet.find_participant_column(ctx.header_row, ctx.id_cell, token)
                if col_idx is not None:
                    if col_idx < len(ctx.header_row):
                        chosen_ids.append(utils.normalize_participant_id(ctx.header_row[col_idx]))
                    else:
...
def browse_spreadsheet(sheet: Any) -> None:
    ...
    num_participants = spreadsheet.get_num_participants(header_row, id_cell, sheet.col_count)
