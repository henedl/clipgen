"""The interactive application loop: spreadsheet opening and mode dispatch.

Entered from `cli.main` when no CLI mode flag was given, i.e. the prompt-driven
session a user gets from a bare `uv run clipgen.py -s SHEET`. Owns the mode
alias table, the `open_spreadsheet_by_*` / `select_spreadsheet` helpers (also
called by the Studio server's spreadsheet-picker routes), and
`run_interactive_mode`, which loops until the user quits.

Sibling split: `interactive.py` holds the *prompt* primitives (line/range/cell
selection, browse mode); this module holds the loop that calls them and the
per-mode handlers they feed. The repo-root `clipgen.py` is only a launcher.

Data flow: Spreadsheet -> Worksheet -> clip records (cell, desc, study,
participant, category; 'times' added by prepare_clip) -> ffmpeg clips or reel.

Full unicode/UTF-8 is supported throughout for study names, participant IDs,
category names, descriptions, and file paths.
"""
