...
- [x] **Suggested threshold.** With clearly separated populations, the midpoint of the gap is a trivially computable suggestion. _Implemented (frontend-only): single-tool calibratable tools (change/similarity/text/template/flow/numbers/inactivity) show a dashed marker + "Apply" badge for a step-aligned cutoff that satisfies the pins — midway through the gap when both polarities are pinned, otherwise hugging the edge of the pinned cluster (pinning only expected positives is a primary workflow, so negatives are never required). Overlapping populations, or a gap too narrow / out of range, show a diagnostic note instead. `_calSuggest`/`_calApplyValue`/`_calBuildTrack` in `screenspace.js`. Multitool, color, and scene excluded._
