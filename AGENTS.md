...
- **Linting/formatting**: Run `uv run ruff check --fix && uv run ruff format` after editing Python files. Run `uv run ty check` for type checking. After editing frontend JavaScript under `assets/web/`, run `bun run lint:js`; use `bun run lint:js:fix` for safe auto-fixes.
