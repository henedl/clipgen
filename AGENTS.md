...
- The Overview page (`/overview/`, [overview.py](overview.py)) hosts three cohort-level tabs as a hub+satellite group: `overview.js` (hub: shared `state`, memoized `ensureData()` bootstrap, tab switching) plus `overview-{map,convergence,metadata}.js` sharing state through `window.ClipgenOverview` (OV). **No pollers** — the hub bootstrap-fetches cross-prefix (`../studio/api/sheet`, `../studio/api/sheet/baseline`, `../screenspace/api/events?excluded=false`, `../transcripts/api/marks`) once, memoized; a Refresh action re-runs it. All four fetches degrade gracefully with no spreadsheet/scans/transcripts. The convergence-offsets routes (`GET/PUT /overview/api/convergence/offsets`) live on the overview blueprint so the satellite's page-relative `apiGet("api/...")` calls resolve; sheet/baseline/thumbnail stay on the studio blueprint, called `../studio/api/...`. The Map tab vendors Three.js r147 UMD (`assets/web/vendor/`, the only third-party JS — see vendor/README.md; do not upgrade, newer releases are ESM-only). Convergence's dispatch-to-queue and Metadata's filter-carrying drill-downs were dropped in the move (Studio-page state; drill-downs are plain `/studio/` links now). Guards: `tests/test_overview_frontend_source.py` (no `_studio` refs, script order, ES5) + the `overview*.js` group in `tests/test_frontend_satellite_wiring.py`.
