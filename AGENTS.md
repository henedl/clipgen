...
- **Debug:** Set `config.DEBUGGING = True` to enable icecream output, skip ffmpeg execution paths in [video.py](source/video.py), and return stub transcript results in [transcripts.py](source/transcripts.py) without loading a Whisper model (the speaker pass in [speakers.py](source/speakers.py) then alternates two labels instead of running the ONNX model, and the redact pass in [redact.py](source/redact.py) tags any `@` token as EMAIL instead of loading the TFLite model).
...
- **CSS design tokens:** shared design values come from `assets/web/tokens.css` (its header maps the families). No new raw `px`/`rem`/hex/durations/z-index in page CSS and no redefining shared tokens; convert touched values when editing — `tests/test_css_token_discipline.py` ratchets the per-file counts. In JS, read colors via `getComputedStyle(...).getPropertyValue(...)`, never hardcoded hex — `tests/test_js_color_discipline.py` ratchets the per-file counts.
...
- Manifest-driven state persistence: every tool keeps its project state as one section of the output dir's `clipgen.json` (`clips`, `screenspace`, `transcripts`, `composer`, `workflows`, `stashes`, `convergence`) through `manifest.load_manifest_section` / `save_manifest_section`, load-on-startup, save-after-mutations. A section is dropped when empty; the file disappears with its last section. Writers also hold a cross-process file lock (`utils.file_lock`; lock files sit under the config dir's `locks/`), so a CLI run and a Studio server can share one output dir.
- No hardcoded version numbers in evergreen docs (AGENTS.md, README.md). Reference `build/VERSION` or call `utils.get_version()` instead; `tests/test_source_conventions.py` checks.
...
- **All Python modules live in [source/](source/); the repo root holds only the `clipgen.py` launcher.** `source/` is not a package — it goes on `sys.path` and modules import by bare name (`import config`). A new module must also be listed in `pyproject.toml` `[tool.setuptools] py-modules`, or installed/frozen builds die with `ModuleNotFoundError` (source-tree pytest masks the omission; `tests/test_packaging.py` guards it). Repo-root paths resolve through `utils.get_bundled_assets_root()`, never a hand-rolled `Path(__file__).parent` (`tests/test_source_conventions.py` bans it). God-file splits: [agents/skills/split-module/SKILL.md](agents/skills/split-module/SKILL.md).
...
- Studio Build Reel uses two paths: `/studio/api/reel` (spreadsheet-only queues; re-resolves cells from the sheet, ignores panel order) and `/studio/api/reel-direct` (intake/mixed queues; panel order, no highlights). Details in the route docstrings in [server.py](source/server.py).
