...
- **Be concise.** Comment blocks are <= 7 words, function names <= 4 words. User-facing message strings should be <= 10 words. Use an active voice, no stage performances, and pick the most common word when choosing among alternatives. Legacy code that breaks these limits gets redone as encountered: delete the old comment or name and write a new one from scratch, never trim word-by-word.
