...
- Screenspace's browser UI is a `screenspace.js` hub plus `screenspace-{tasks,results,overlay,overlay-interaction,timeline,model-view,multitool-params,calibration,color}.js` satellites sharing state through the `window.ClipgenScreenspace` (SS) namespace (mirrors studio.js + convergence/metadata). When carving more out, audit hub references to satellite-internal `var`s — not just the SS read/write contract and function refs: a hub reference to a `var` that moved into a satellite throws a runtime `ReferenceError` that `node --check` can't catch (this shipped the `_calibrationGen` frame-load bug and the multitool drag-state vars). Route shared mutable state through `state.`/`SS.state` (the tasks/results carve moved `resultsRequestVersion`, `suppressCalibrationRefresh`, `heatmapOverlayRequestVersion` onto `state` because the hub and two satellites all touch them) or guarded `SS.` accessors (`if (SS.fn) SS.fn()`). **Load order is a contract:** a satellite that *destructures* another's published fn at load time must load after it (`screenspace-model-view.js` loads first after the hub because overlay/calibration/tasks/multitool-params destructure its `_overlayEligibleForActiveTool`/`_previewRegionRef`/`_updateMinAreaReadout`; `screenspace-tasks.js` loads before the satellites that destructure its `findTask`/`restoreTaskToWorkflow`/`setInputValue`/`syncValueDisplays`; `screenspace-results.js` loads last). When the owner loads *later*, call it late-bound as `SS.fn(...)` at the call site instead (tasks reaches results' `renderResults`/`loadAndShowResults`, and the color satellite's `setTargetColor`, this way). The procedure for carving a new satellite (delegators, state routing, load order) is [agents/skills/carve-satellite/SKILL.md](agents/skills/carve-satellite/SKILL.md); `tests/test_frontend_satellite_wiring.py` fails CI on any bare cross-file call that has no delegator/import (covers Screenspace, Transcripts, Studio, Workflows).
...
- Studio's browser UI follows the same axis: a `studio.js` hub plus `studio-{intake,generate,trim,stash,scrubber}.js` satellites (Screenspace + Transcript intake panels; the streaming generate flow; the duration-badge trim pop-over; the reel/artifact stash panels; hover-to-scrub card wiring) sharing state through `window.ClipgenStudio` (STUDIO), with same-named guarded delegators in the hub (`function pollIntakeEvents(){ return STUDIO.pollIntakeEvents && STUDIO.pollIntakeEvents.apply(null, arguments); }`). The old `window._studio*` legacy globals are **gone**. Their readers (metadata.js/convergence.js) moved to the Overview page. Deliberately **kept in the hub** despite living in the intake region: the `ss*Thumb*` lazy-thumbnail cluster (shared with the hub's queue cards; the intake satellite needs `ssClearPending`, the stash satellite `ssEnqueueThumbCustom`, via STUDIO). `buildXrefBadges`/`xrefBadgeIcon` are utils.js ambient globals (shared with Overview). **Same `var`-ReferenceError gotcha** (the stash carve moved the hub drop-target callbacks that wrote the satellite-private `_justStashedId` into the satellite as late-bound `STUDIO.stashDropReel`/`.stashDropArtifacts` handlers rather than leave a bare cross-file write). The boot block's two `initIntakePanel(SS_INTAKE/TR_INTAKE)` calls were folded into a satellite `initIntake()` so the configs never leave the satellite. Tests: `tests/test_studio_frontend_source.py` globs `studio*.js` so assertions stay valid wherever a function lives.
- The Overview page (`/overview/`, [overview.py](overview.py)) hosts three cohort-level tabs as a hub+satellite group: `overview.js` (hub: shared `state`, memoized `ensureData()` bootstrap, tab switching) plus `overview-{map,convergence,metadata}.js` sharing state through `window.ClipgenOverview` (OV). **No pollers.** The hub bootstrap-fetches cross-prefix (`../studio/api/sheet`, `../studio/api/sheet/baseline`, `../screenspace/api/events?excluded=false`, `../transcripts/api/marks`, plus its own `api/friction-moments`) once, memoized with a `dataVersion` counter (the tabs' staleness banners compare versions, never lengths); a Refresh action re-runs it. All fetches degrade gracefully with no spreadsheet/scans/transcripts. The Map's observation features come from **sheet timestamps**, not clip artifacts: server.py injects `_sheet_observation_rows()` via `overview.configure()` (the sheet lives in studio-blueprint process state: injection, not an import cycle). The convergence-offsets + friction-moments routes live on the overview blueprint so the satellites' page-relative `apiGet("api/...")` calls resolve; sheet/baseline/thumbnail stay on the studio blueprint, called `../studio/api/...`. The Map tab vendors Three.js r147 UMD (`assets/web/vendor/`, the only third-party JS; see vendor/README.md; do not upgrade, newer releases are ESM-only). `/transcripts/#P07`-style hashes (emitted by the Map explain panel, parsed by `clipgenHashParticipant()` in utils.js) pre-select a participant on those pages. Convergence's dispatch-to-queue and Metadata's filter-carrying drill-downs were dropped in the move (Studio-page state; drill-downs are plain `/studio/` links now). **The generic `.hidden` utility is per-page CSS.** Any JS moved onto this page needs its toggled classes present in overview.css (shipped the always-visible-banners bug). Guards: `tests/test_overview_frontend_source.py` (no `_studio` refs, script order, ES5, `.hidden`) + the `overview*.js` group in `tests/test_frontend_satellite_wiring.py`.
