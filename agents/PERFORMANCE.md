...
| `TITLECARD_ENCODE_PRESET` / `TITLECARD_ENCODE_CRF` | `veryfast` / `20` | x264 preset + CRF for title/endcard generation and the card wrap. Wrapping re-encodes the whole clip body, so the preset dominates titlecard time — `veryfast` is several times faster than libx264's `medium` default. Drop to `superfast`/`ultrafast` for more speed (larger files), or lower the CRF for higher quality. (A future win is copying the clip bitstream via the concat demuxer like reels do, avoiding the body re-encode entirely.) |
