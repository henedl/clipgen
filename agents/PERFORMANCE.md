...
| `TRANSCRIBE_VAD_FILTER` (+ `_THRESHOLD` / `_SPEECH_PAD_MS` / `_MIN_SILENCE_MS`) | `True` (0.3 / 400 / 2000) | Silero VAD skips long silence, the big win on mostly-quiet UX recordings. Defaults are recall-safe (permissive threshold + boundary padding). If real words drop, lower the threshold or raise the pad rather than disabling VAD. |
