...
- **Absolutely-positioned dropdowns size to their trigger, not their content**: an `position: absolute` panel shrink-to-fits against its `position: relative` containing block, so a menu anchored to a ~2rem icon button collapses to its `min-width` and wraps every label longer than that — mid-phrase, and only for the longest one, which is why it reads as "that one label is broken" rather than a layout rule. Pair `width: max-content` with the `min-width`, plus a `max-width` cap so a future label can't run off-screen. Hit `.topnav-qa-panel` and `.rp-export-menu` independently in one change (`5480818`); invisible to `/check`, so **look at the screenshot** after adding or lengthening any menu label.
