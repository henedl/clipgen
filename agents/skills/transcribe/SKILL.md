...
Redact: `TRANSCRIBE_REDACT` (Studio: **Transcription → Redaction**, off by default) sets the default; the **Redact** tab (beside Summary and Friction) switches it per participant and persists on the manifest entry; the tab lists every placeholder with its label, surface, count and lowest confidence; a click jumps to the line and the undo arrow restores that surface everywhere (redact it again from the same row). Clicking a chip in the transcript opens the tab. The pass needs the Redact model (Desert Ant Labs, ~25 MB), downloadable from the tab or the settings group; until then the switch is disabled and `PUT api/redact/<pid>` answers 409 `model_missing`. Detected names, contacts, addresses and ids show as `[GIVEN_NAME_1]`-style chips in the page (hover for label and confidence), and the placeholders flow into VTT/SRT/Markdown, search, marks, `--export`, and the summary/citations/friction/report prompts. The text itself is never rewritten: switching off restores it. `TRANSCRIBE_REDACT_MIN_SCORE` is the confidence floor; `TRANSCRIBE_REDACT_ORG` also tags organisation names. Editing a line while redaction is on is refused (turn it off first).
