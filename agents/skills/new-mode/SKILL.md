...
6. **New top-level modules** (if any)
   - If the mode adds a new module, create it in `source/` and list its bare name in `pyproject.toml [tool.setuptools] py-modules`. Source-tree imports mask the omission but `uv pip install .` breaks. Guarded by `tests/test_packaging.py`. See [agents/skills/split-module/SKILL.md](../split-module/SKILL.md).
