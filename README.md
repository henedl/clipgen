...
2. Install ffmpeg: either manually or by `scripts/install-ffmpeg-ollama.sh` (macOS/Linux) or `scripts/install-ffmpeg-ollama.bat` (Windows).
...
The Transcripts thinking agents and Overview reports use a local [Ollama](https://ollama.com/download) server. clipgen offers to install Ollama and pull the models the first time you use an AI feature, and starts `ollama serve` for you. Transcription itself does not need Ollama.
...
- **Studio** — click timestamp cells in your spreadsheet to generate clips, screenshots, and GIFs, and build reels interactively.
- **Screenspace** — automated visual analysis of recordings: draw a region and detect color, change, text, motion, and more across the video.
- **Transcripts** — local transcription, with optional AI summaries.
- **Composer** — cut and annotate moments that aren't in the spreadsheet, on a timeline over the full recording.
...
clipgen's own source is MIT. The desktop builds bundle third-party software under MIT, BSD, Apache-2.0, HPND, MPL-2.0, LGPL, SIL OFL, and GPL terms. Full notices are in [build/THIRD-PARTY-LICENSES](build/THIRD-PARTY-LICENSES).
