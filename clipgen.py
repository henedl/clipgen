	...
	while True:
		...
		for i in range(0, len(timesList)):
			...
			for j in range(0,len(timesList[i]['times'])):
				...
				try:
					# TODO This is actually terrible. We can extract the study number in some better way than this (filter > list > list index 0)
					vidName = set_filename('[Study ' + list(filter(str.isdigit, timesList[i]['study']))[0] + '][' + timesList[i]['category'] + '] ' + timesList[i]['desc'] + FILEFORMAT)
