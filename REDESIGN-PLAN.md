...
| 4 | Screenspace | Visual restyle (no layout change): hue-dotted workflow tabs, ConfBar in results, circular legend dots, mono inline slider values | **Done** |
| 5 | Transcripts | Visual restyle (no PiP/width change): AnnoPopover-style markPopover, 14 px anno dots + inline color-mix anno-badge, summary card chrome, mono+tnum participant pills, taller search input | **Done** |
| 6 | Polish | Card-size unification (drop `.queue-card { width }`; bottom-strip cards promoted to `.clip-card.size-md`); light-theme parity sweep (`--stream-{sheet,screenspace,transcript}` tokens; metadata.css + studio.css + transcripts.css color-mix swaps); floating-nav scroll-under restructure (TopNav + subheader + Transcripts pillBar → `position: fixed`; per-surface `padding-top: 92/148px` so visible content sits below chrome and scrolled content slides *under* the glass; sheet thead bumps to `top: 92px`); detector-icon light pass (verified — already normalized via base class); card-scrubber audit (verified — class-agnostic); viewer/gallery tokens-only (`tokens.css` link added with `data-dev-only`). **Deferred (per user direction):** detector glyph revision (design-blocked); animation finalization (design-blocked); Screenspace flat-scroll layout; Transcripts sticky-PiP option. | **Done** |
...
### Sessions 4 + 5 — Screenspace and Transcripts visual restyle (2026-05-05)

**Branch:** `henedl/redesign-pass-4-5` (one combined PR)

**Scope decision:** User reduced the original "Safe layout" port for both passes down to a pure visual restyle — keep the existing top/bottom drag-resize layout in Screenspace and the existing fixed-position drag/dock PiP in Transcripts. The argument: those subsystems already work and the prototype's flat-vertical-scroll Screenspace and sticky-PiP Transcripts would lose features without obvious gain. Pass 6 captures the deferred layout-structure work as optional follow-ups.

**Shipped — Screenspace:**

- **Workflow tab hue dot.** Added `.wf-tab::before` pseudo-element (6 px circle) before each tab label, color-driven by per-detector `--wf-tab-hue` CSS variables that resolve to the existing `--color-task-{type}` tokens. Inactive opacity 0.5 → active opacity 1 via `.wf-tab.active::before`. No HTML changes — purely a CSS addition. Also filled in the missing `[data-type="inactivity"]:hover, .active` rule so all 11 detectors have consistent active-state styling.
- **ConfBar pattern in results.** New `buildConfBar(value, type)` helper in `screenspace.js` renders a hue-tinted 4 px bar with opacity = 0.4 + value × 0.6 (mirrors the prototype's `ConfBar` component). `.result-bar` CSS reflowed from fixed 40 px width to `flex: 1; min-width: 80px; background: var(--bg-input)`. Bars now show on `change` (existing), `flow` (existing), and added to `similarity`, `text`, `template`, `scene` — all the score-bearing detector outputs.
- **Legend dot circular.** `.legend-dot` border-radius `var(--radius-sm)` → `50%` so the timeline legend dots match the new design language's circular hue swatches.
- **Slider mono value alignment.** `.param-value` got `text-align: right` so multi-digit confidence/threshold values line up tidily next to range inputs (existing custom slider styling already met the prototype's intent).
- **No button migration.** Considered swapping the toolbar `.btn` / `.ss-btn` / `.rp-icon-btn` patterns to `createBtn`. Decided against — the existing CSS is already cohesive (transparent → surface bg, accent hover, disabled-tooltip pattern) and migrating to `cg-btn` would just trade one consistent system for another with no harmony win. Logged the decision in the task description.

**Shipped — Transcripts:**

- **markPopover → AnnoPopover anatomy.** Width clamped to 180 px, padding to 10 px, border to `1px solid var(--border-strong)`, radius to 6, shadow to `var(--shadow-pop)`. `.mark-cat-pill` shrunk from 20 → 18 px circle, with the prototype's two-layer treatment on active (`2px solid #fff` border + `1px var(--border-strong)` outline at offset 1). Label input → 26 px tall, 12 px font, `var(--bg-input)` bg, 4 px radius. Remove button → 24 px tall, 11.5 px font, transparent bg, bottom-right alignment via `align-self: flex-end`. Categories still come from the manifest — no JS-model change.
- **Inline anno-badge in segment text.** New `.segment-anno-badge` CSS rule consumes per-row CSS variables (`--anno-badge-fg/bg/border`) so each badge can use the mark's category color via `color-mix(in oklch, <color> 18%/50%, transparent)` for bg/border. `renderSegments()` injects the badge HTML inside `.segment-text` before the word spans when the mark has a non-empty label. Click-to-jump word interaction unchanged.
- **Segment mark dot.** `.segment-mark` reshaped to 14 × 14 px (was 12 + 2 px border = 14 effective), border `1.5px var(--border-strong)` when unmarked, transparent border + filled background when marked. Removed the 2 px border-collapse hack in favor of clean radii.
- **Summary card chrome.** `#summarySection` became a card: `var(--bg-elevated)` background, `1px solid var(--color-border)`, 6 px radius, `margin: var(--space-3) var(--space-4)`. Replaced the old top/bottom-bordered open-flow style. `.summary-icon` shrunk to 13 px to match prototype's sparkles size; `.summary-title` letter-spacing bumped 0.05 → 0.08em; `.summary-edit-textarea` retuned to 80 px min-height + `var(--bg-input)` bg + 4 px radius + 13 px font (matches prototype editable mode). The mid-prototype "· generated N min ago" tail was skipped — the data plumbing isn't there today and adding it crosses out of "visual restyle" scope.
- **Participant pill mono+tnum.** `.pill` switched to `var(--bg-elevated)` bg, `var(--color-text-dim)` text, `--bg-active` + `--border-strong` on active — aligning with `.participant-pill` from primitives.css without actually loading the primitive. `.pill-id` switched to `var(--font-mono)` + `font-feature-settings: "tnum"` + 11.5 px size. Decided NOT to swap the rich `.pill` structure (progress overlay, status-driven trigger glyph, chevron-to-options) for `createParticipantPill` — that primitive is just a label button and would lose the retranscribe/cancel actions and progress visualization. Visual alignment achieved without a JS rewrite.
- **Search input height.** `#searchInput` → 32 px tall, 12 px horizontal padding, `var(--bg-elevated)` bg, 4 px radius, 13 px font. Matches the prototype's search input chrome.

**Verified:**

- `node -c` clean for `screenspace.js`, `transcripts.js`, `primitives.js`.
- `uvx ruff check`, `uvx ruff format --check` — all green, 50 files already formatted.
- `uvx ty check` — 13 pre-existing diagnostics (pytest module resolution; unchanged from earlier sessions).
- `uv run --extra dev pytest -c tests/pytest.ini` — 733 tests pass, 0 regressions.
- `uv run clipgen.py --screenspace -i /tmp/cg-empty -o /tmp/cg-empty` boots cleanly; tokens.css, topnav.css, screenspace.{css,js}, primitives.{css,js} all serve 200.
- TopNav, theme toggle, settings modal all unaffected.

**Deferred to pass 6 (now explicitly tracked in the roadmap row):**

- Screenspace top/bottom drag-resize → flat vertical-scroll layout (Safe variant per prototype). User decision to keep the working drag pattern was the simplifier this session; pass 6 can revisit if the drag affordance turns out to be load-bearing for fewer users than expected.
- Transcripts sticky-PiP option. Current fixed-position drag/dock PiP retained.
- Card-size token unification across `.clip-card` / `.transcript-card` / `.queue-card`.
- Light-theme parity check.
- Animation finalization with design lead.
- Detector iconography revision (handoff flagged "not final").
- Floating-nav scroll-under restructure (frameset.app/search reference).

**Surprises / lessons:**

- **`categoryHue()` already covered the detector hue map.** The plan called for adding a new `ssDetectorHue(type)` helper mirroring the prototype's `detHue()` table. But pass 1 had already added `categoryHue(label)` and `categoryColor(label, alpha)` in `utils.js` keyed off `CATEGORY_HUES` — and that table already includes Multitool/Color/Change/Similarity/Text/Numbers/Timelapse/Template/Flow/Scene/Inactivity entries with the prototype's exact hue values. No new helper needed; just use `categoryHue(type)` from utils. Saved ~25 lines and one indirection.
- **Per-detector CSS variables beat 11 dot-color rules.** First instinct on the workflow tab dot was to write 11 rules like `.wf-tab[data-type="X"]::before { background: var(--color-task-X) }`. Instead, set `--wf-tab-hue: var(--color-task-X)` once per `data-type`, then a single `.wf-tab::before { background: var(--wf-tab-hue) }`. Adding a new detector down the road is one one-line rule, not two.
- **Neither Screenspace nor Transcripts loads `primitives.{css,js}` today** — only Studio does. Caught this when reviewing what tokens my CSS used. All my changes stuck to `tokens.css` tokens (`--bg-elevated`, `--bg-input`, `--border-strong`, `--shadow-pop`, etc.) which are global. The `.cg-btn` / `.participant-pill` selectors from primitives wouldn't have applied; this was the practical reason to keep the `.pill` structure intact and just retoken its CSS instead of swapping to `createParticipantPill`. Pass 6 might want to add the primitives links to both pages as a no-op enabler.
- **Inline color-mix via per-row CSS variables.** The anno-badge needs per-instance bg/border/fg colors. Setting three inline CSS variables on the `<span>` (`--anno-badge-fg/bg/border`) from JS, then having one CSS rule reference them, beats either (a) inline `style=` with three full property declarations or (b) a per-color class. Generalises to other "tinted by data" patterns.
- **User scope reduction mid-plan.** Original plan had a bigger Pass 5 — narrow column to 880 px and convert PiP to sticky. User pulled both back: "we already have a working PiP system, let's keep using that" + "same column width is okay". Updating the plan file before coding meant zero rollback. Same lesson as session 3.

### Session 6 — final polish (2026-05-05)

**Branch:** `henedl/redesign-pass-6`

**Scope decision:** Of the nine pass-6 candidates in the roadmap row, six were taken in scope per user direction; two were design-blocked (detector glyph revision, animation finalization); two were explicitly user-deferred from prior passes (Screenspace flat-scroll, Transcripts sticky-PiP). The biggest item — the floating-nav scroll-under restructure — went from "visual-only chrome with empty space behind" to actually working: chrome is now `position: fixed` and content slides *under* the glass when scrolled.

**Shipped:**

- **Card-size unification.** Dropped `.queue-card { width: var(--card-width-queue) }` from `studio.css`. Stopped adding `.queue-card` class on Studio intake `.clip-card` / `.transcript-card` (the source of the session-2 collision where `.queue-card`'s 140 px overrode `.clip-card.size-lg`'s 180 px). Bottom-strip artifact + reel cards now render with `clip-card size-md` (150 px) alongside the existing `.queue-card` chrome (border, hover shadow, remove button, state classes still ride on `.queue-card`). Extended the `.queue-card:hover .xref-badge-stack` rule to also cover `.clip-card:hover` and `.transcript-card:hover` so badge-spread-on-hover keeps working on intake cards. Dropped unused tokens `--card-width-queue` and `--card-width-stash`; kept `--card-width-preview`.
- **Light-theme parity sweep.** Added `--stream-sheet`, `--stream-screenspace`, `--stream-transcript` tokens to `tokens.css` (constant across themes — semantic colors that need to stay distinguishable on either background). Replaced 8 hardcoded `rgba(...)` sites in `metadata.css` (freshness banner, stale banner, three stream indicators, outlier tint, two collision bars), 1 in `studio.css` (intake source badge), and 1 in `transcripts.css` (search highlight) with `color-mix(in oklch, var(--stream-...) <alpha>%, transparent)`. Theme-toggle sun/moon icons (`#fde68a` etc.) and video-chrome backgrounds (`#0a0a0a`) intentionally left hardcoded — those represent visual constants (sun is yellow, video chrome is always-dark) that shouldn't flip with the theme.
- **Floating-nav scroll-under restructure.** `topnav.css` `.topnav { position: sticky; top: 0 }` → `position: fixed; top: 0; left: 0; right: 0`. `tokens.css` `.subheader, #studioSubheader, #ssSubheader, #trSubheader { position: sticky; top: 48px }` → `position: fixed; top: 48px; left: 0; right: 0`. Per-surface scroll-container padding so visible content starts below chrome while scrolled content slides under it: Studio's `#sheetGrid` gets `padding-top: 92px`; sheet thead's sticky offset bumps from `top: 0` → `top: 92px` to dock right below chrome; `#intakePanel`, `#trIntakePanel`, `#convergencePanel`, `#metadataPanel`, `#studioSidebar` all get a 92 px top inset. Screenspace's `#ssMain` gets `padding-top: calc(var(--space-4) + 92px)`. Transcripts' `#trPillBar` flips from `position: sticky; top: 92px` → `position: fixed; top: 92px` (joining the chrome strip), and `#trMain` gets `padding-top: 148px` (= 48 + 44 + 56 chrome height). Bottom-panel resize on Studio works unchanged (it sits at the bottom edge, never under chrome).
- **Viewer + Gallery tokens-only.** Added `<link rel="stylesheet" href="tokens.css" data-dev-only>` to `viewer.html`, `gallery.html`, `timeline-viewer.html`. The `data-dev-only` attribute is stripped by `viewer.py`'s inlining regex on export (lines 250–255), so live dev preview gets tokens while standalone exported HTML stays self-contained without broken external links. Audit of hardcoded colors in `viewer.css` and `gallery.css` found everything intentional (theme-toggle sun/moon icons, lightbox controls overlaid on dark backdrop, video-thumbnail gradients) — no token-swaps needed.
- **Detector-icon light pass + card-scrubber audit.** Both came back as positive null results. Detector icons (`.ss-task-icon--*` rules in `screenspace.css`) are already fully normalized via the base `.ss-task-icon` class — same `--icon-size-sm`, `mask-size: contain`, `mask-position: center` for all 10 detector rules. Timelapse intentionally has no result-row icon per AGENTS.md (it's an output tool, not a per-frame detector). Card-scrubber (`card-scrubber.{js,css}`) is class-agnostic, attached imperatively per element; it's currently a dormant utility (no HTML file loads it, no JS calls `clipgenCardScrubber.attach()`), kept available for future wiring. `SS_THUMB_SELECTOR` in `studio.js` already covers all three card-thumb classes.

**Verified:**

- `node -c` syntax-check clean for `studio.js`, `screenspace.js`, `transcripts.js`, `primitives.js`.
- `uvx ruff check` and `uvx ruff format --check` — green, 50 files already formatted.
- `uvx ty check` — all checks passed.
- `uv run --extra dev pytest -c tests/pytest.ini` — 733 tests pass, 0 regressions.
- `uv run clipgen.py --screenspace -i /tmp/cg-empty -o /tmp/cg-empty` — boots cleanly; tokens.css, topnav.css, screenspace.{css,js}, primitives.css all serve 200; `/screenspace/` and `/transcripts/` routes return 200.

**Deferred (out of scope this pass):**

- Detector glyph revision — design-blocked (`redesign/README.md` line 176: "revisit with design before shipping").
- Animation finalization — design-blocked (`redesign/README.md` line 201: "not finalized").
- Screenspace top/bottom drag-resize → flat vertical-scroll layout — user-deferred from pass 4 ("keep the working drag pattern").
- Transcripts sticky-PiP layout — user-deferred from pass 5 ("we already have a working PiP system"). Transcripts gets the chrome restructure but the inner scroll behaviour for transcript segments stays bounded by `#transcriptSection`'s scroll viewport (below `#videoSection` + `#summarySection`); the visual chrome glass shows blur over `#trMain`'s panel-bg in the top 148 px region.

**Surprises / lessons:**

- **Padding belongs on the scroll container, not its parent.** First attempt put `padding-top: 92px` on `#sheetPreview` (the flex parent of `#sheetGrid`). That pushed the table down visually but broke "content scrolls under chrome" — because the padding was outside the scroll viewport, scrolled content stopped at the scroll container's top edge (which sat at viewport y=92, below chrome). Moving the padding onto `#sheetGrid` itself fixed it: padding is part of the scrollable area, so it scrolls up with content, and content slides through the padding into the chrome region.
- **Sticky-element offset bumping isn't always needed.** With chrome → fixed and `padding-top: 92px` on the scroll container `#sheetGrid`, sticky thead at `top: 0` would stick at the scroll container's *content* top edge, which is at viewport y=0 — UNDER chrome. Bumping thead to `top: 92px` makes it stick at viewport y=92 (just below chrome). Same logic for `#trPillBar`: in the original (sticky) model, `top: 92px` was correct; in the new (fixed) model the pill bar is itself fixed at top:92, so its sticky offset disappears entirely.
- **`.queue-card` had the *width* problem, not the chrome problem.** Session-2's lesson logged the 140px-vs-180px collision as a known issue. The fix was surgical: drop just the `width` rule, keep all other queue-card visuals (border, hover shadow, remove button, state classes). Bottom-strip cards continue to ride on `.queue-card` for those affordances; the new `.clip-card.size-md` only controls width. Trying to fully migrate state classes / remove button onto `.clip-card` was tempting but would have ballooned scope without visual payoff this pass.
- **`data-dev-only` is the right pattern for dev-only links.** Adding `<link rel="stylesheet" href="tokens.css">` to `viewer.html` without `data-dev-only` would have leaked into exported standalone HTML as a broken external reference (since `viewer.py` only swaps the canonical `viewer.css` link with an inline `<style>` block). The existing `data-dev-only` regex in `viewer.py` (lines 244–255) already covers script and link tags. Realised mid-edit that `gallery.html` had been doing this incorrectly since pass 1 — its `tokens.css` link was leaking into exports. Fixed in the same pass.
- **Audits often resolve as positive null results.** Card-scrubber, detector iconography, light-theme parity (mostly): each came in expecting concrete change-targets and resolved as "already correct" or "intentional". Only metadata stream indicators turned out to be a real tokenization win. Future passes should bias toward bigger upfront audits before scope-locking — the user was already correct that detector glyphs needed design input, but I had to verify the icon CSS was normalized to confirm "no glyph swaps" was the right call.
